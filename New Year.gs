function newYear() {
  ...
  var input, check, year, newYear, row, numRows, names;
  while (!check) {
    ...
    if (input.getResponseText() == '') { ui.alert('Blank', 'Password cannot be blank! Please try again', ui.ButtonSet.OK); continue; }
  ...
  for (var i = 1; i < sheets.length; i++) {
    for (var j = 0; j < dealers.length; j++) {
      ...
      row = names[1];
      numRows = parseInt(names[2]) - parseInt(row) + 1;
      sheets[i].getRange(row, 3, numRows, 3).setValue('');
    }
    sheets[i].setName(sheets[i].getSheetName().replace(year, newYear));
  }
}
